...
The application will use SQLite, a lightweight relational database. The schema will include tables for buildings (with identifiers, names and geographic coordinates), rooms (referencing buildings and including names, capacities and availability), events (names, descriptions, dates, times and locations), bookings (linking users to rooms with start and end times) and users/roles. SQLite is file‑based and easy to deploy, which makes it appropriate for a prototype; it integrates seamlessly with Node.js via libraries such as \`sqlite3\`.
